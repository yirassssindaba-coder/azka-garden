...
import { customerServiceBot, ChatMessage } from '../../ai/chatbot/CustomerServiceBot';
...
const ChatBot: React.FC = () => {
  ...
  const initializeChat = async () => {
    try {
      const newSessionId = await customerServiceBot.startChat();
      setSessionId(newSessionId);
      
      // Get initial messages
      const history = await customerServiceBot.getChatHistory(newSessionId);
      setMessages(history);
    } catch (error) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const responses = await customerServiceBot.processMessage(sessionId, userMessage);
      const updatedHistory = await customerServiceBot.getChatHistory(sessionId);
